...
public class GraphViewerPanel extends JPanel {
	    ...
	    public final String SPRITE_APCLreplaced = "sprite.apClreplaced";

	    public final String SPRITE_BNDORD = "sprite.bndOrd";

	    public final String SPRITE_FRGID = "sprite.fragId";

	    public final String SPRITEATT_UICLreplaced_APCLreplacedSRC = "apLabelSRC";

	    public final String SPRITEATT_UICLreplaced_APCLreplacedTRG = "apLabelTRG";

	    public final String SPRITEATT_UICLreplaced_BNDORD = "bndTypLabel";

	    public final String SPRITEATT_UICLreplaced_FRGID = "molIdLabel";
	    ...
	    private void updateGraphStyle() {
	        cssStyle = "graph {" + "fill-color: #D9D9D9; " + "} " + "node {" + "shape: rounded-box; " + "size: " + GUIPreferences.graphNodeSize + "px, " + GUIPreferences.graphNodeSize + "px; " + "fill-mode: plain; " + "fill-color: grey; " + "stroke-color: #1D2731; " + "stroke-width: 2px; " + "stroke-mode: plain; " + "text-mode: normal; " + "text-style: normal; " + "text-size: " + GUIPreferences.graphLabelFontSize + ";" + "} " + "node.fragment {" + "fill-color: #4484CE; " + "} " + "node.scaffold {" + "fill-color: #F53240; " + "} " + "node.cap {" + "size: " + (GUIPreferences.graphNodeSize * 2 / 3) + "px, " + (GUIPreferences.graphNodeSize * 2 / 3) + "px; " + "fill-color: #57BC90; " + "} " + "node.rcv {" + "size: " + (GUIPreferences.graphNodeSize * 2 / 3) + "px, " + (GUIPreferences.graphNodeSize * 2 / 3) + "px; " + "fill-color: #F19F4D; " + "}" + "node.ap {" + "shape: circle;" + "size: " + (GUIPreferences.graphNodeSize * 1 / 3) + "px, " + (GUIPreferences.graphNodeSize * 1 / 3) + "px; " + "stroke-width: 1px; " + "text-mode: hidden;" + "fill-color: #FECE00; " + "}" + "node:selected {" + "stroke-color: darkgreen;" + "fill-color: green;" + "}" + "edge {" + "shape: line; " + "size: 1.5px;" + "fill-color: #1D2731; " + "arrow-size: 7px, 5px; " + "}" + "edge.rc {" + "fill-color: #F19F4D; " + "}" + "edge.ap {" + "fill-color: #FECE00;" + "}" + "sprite.edgeLabel {" + "shape: box; " + "size: 0px;" + "text-mode: normal;" + "text-style: normal;" + "text-size: " + GUIPreferences.graphLabelFontSize + ";" + "text-background-mode: rounded-box;" + "text-background-color: #D9D9D9;" + "text-padding: 1px;" + "} " + "sprite." + SPRITEATT_UICLreplaced_APCLreplacedSRC + " {" + "shape: box; " + "size: 0px;" + "text-mode: normal;" + "text-style: normal;" + "text-size: " + GUIPreferences.graphLabelFontSize + ";" + "text-background-mode: none;" + "}" + "sprite." + SPRITEATT_UICLreplaced_APCLreplacedTRG + " {" + "shape: box; " + "size: 0px;" + "text-mode: normal;" + "text-style: normal;" + "text-size: " + GUIPreferences.graphLabelFontSize + ";" + "text-background-mode: none;" + "}" + "sprite." + SPRITEATT_UICLreplaced_FRGID + " {" + "shape: box; " + "size: 0px;" + "text-mode: normal;" + "text-style: normal;" + "text-background-mode: none;" + "}" + "sprite." + SPRITEATT_UICLreplaced_BNDORD + " {" + "shape: box; " + "size: 0px;" + "text-mode: normal;" + "text-style: normal;" + "text-size: " + GUIPreferences.graphLabelFontSize + ";" + "text-background-mode: rounded-box;" + "text-background-color: #D9D9D9;" + "text-padding: 1px;" + "}";
	    }
	    ...
	    private void appendSpritesFromSnapshot(GSGraphSnapshot snapshot) {
	        ...
	        if (snapshot.hreplacedpritesOfType(SPRITEATT_UICLreplaced_APCLreplacedSRC)) {
	            for (Element el : snapshot.getSpritesOfType(SPRITEATT_UICLreplaced_APCLreplacedSRC)) {
	                if (el instanceof Edge) {
	                    Edge ed = graph.getEdge(el.getId());
	                    if (ed != null) {
	                        addSrcApClreplacedSprite(ed);
	                    }
	                }
	            }
	        }
	        if (snapshot.hreplacedpritesOfType(SPRITEATT_UICLreplaced_APCLreplacedTRG)) {
	            for (Element el : snapshot.getSpritesOfType(SPRITEATT_UICLreplaced_APCLreplacedTRG)) {
	                if (el instanceof Edge) {
	                    Edge ed = graph.getEdge(el.getId());
	                    if (ed != null) {
	                        addTrgApClreplacedSprite(ed);
	                    }
	                }
	            }
	        }
	        if (snapshot.hreplacedpritesOfType(SPRITEATT_UICLreplaced_BNDORD)) {
	            for (Element el : snapshot.getSpritesOfType(SPRITEATT_UICLreplaced_BNDORD)) {
	                if (el instanceof Edge) {
	                    Edge ed = graph.getEdge(el.getId());
	                    if (ed != null) {
	                        addBondSprite(ed);
	        ...
	        if (snapshot.hreplacedpritesOfType(SPRITEATT_UICLreplaced_FRGID)) {
	            for (Element el : snapshot.getSpritesOfType(SPRITEATT_UICLreplaced_FRGID)) {
		...
		*/
	    // -----------------------------------------------------------------------------
	    private void addSrcApClreplacedSprite(Edge e) {
	        String sId = "srcApClreplaced-" + e.getId();
	        if (!hreplacedprite(sId)) {
	            Sprite sSrc = sman.addSprite(sId);
	            sSrc.setAttribute("ui.clreplaced", SPRITEATT_UICLreplaced_APCLreplacedSRC);
	            sSrc.addAttribute("ui.label", (String) e.getAttribute("dnp.srcAPClreplaced"));
	    ...
	    // -----------------------------------------------------------------------------
	    private void addTrgApClreplacedSprite(Edge e) {
	        String sId = "trgApClreplaced-" + e.getId();
	        if (!hreplacedprite(sId)) {
	            Sprite sTrg = sman.addSprite(sId);
	            sTrg.setAttribute("ui.clreplaced", SPRITEATT_UICLreplaced_APCLreplacedTRG);
	            sTrg.addAttribute("ui.label", (String) e.getAttribute("dnp.trgAPClreplaced"));
	    ...
	    private void addBondSprite(Edge e) {
	        ...
	        if (!hreplacedprite(sId)) {
	            ...
	            s.setAttribute("ui.clreplaced", SPRITEATT_UICLreplaced_BNDORD);
	    ...
	    private void addFrgIdSprite(Node n) {
	        if (n.getAttribute("ui.clreplaced").equals("ap")) {
	            return;
	        }
	        String sId = "molID-" + n.getId();
	        if (!hreplacedprite(sId)) {
	            Sprite s = sman.addSprite(sId);
	            s.setAttribute("ui.clreplaced", SPRITEATT_UICLreplaced_FRGID);
	            s.addAttribute("ui.label", "MolID: " + n.getAttribute("dnp.molID"));
	     ...
	     */
	    public boolean hreplacedprite(String sprId) {
	        boolean res = false;
	        ...
	        switch(sprType) {
	            case SPRITE_APCLreplaced:
	                for (Node n : getSelectedNodes()) {
	                    for (Edge e : graph.getEdgeSet()) {
	                        if (e.getSourceNode() == n) {
	                            Sprite s = sman.getSprite("srcApClreplaced-" + e.getId());
	                            if (s != null) {
	                                sprToDel.add(s.getId());
	                            }
	                        }
	                        if (e.getTargetNode() == n) {
	                            Sprite s = sman.addSprite("trgApClreplaced-" + e.getId());
	                            if (s != null) {
	                                sprToDel.add(s.getId());
	                            }
	                        }
	     ...
	     */
	    public boolean hreplacedelected() {
	        return mouseManager.hreplacedelected();
	    }
	    ...
	    // -----------------------------------------------------------------------------
	    /**
	     * Manager clreplaced to handle mouse events on graph area
	     */
	    private GraphMouseManager extends DefaultMouseManager {
