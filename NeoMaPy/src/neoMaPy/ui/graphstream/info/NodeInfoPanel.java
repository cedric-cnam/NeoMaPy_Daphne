...
public class NodeInfoPanel extends JPanel {
	...
	public NodeInfoPanel(NeoMaPyGraph graph, SearchPanel search, int width, int height) {
		super();
		this.graph = graph;
	...
	void setNodeInfo(String nodeId) throws BadLocationException, IOException {
		...
		// append("<span style=\"font-size:8px;\">NodeId: "+ID+"</span>", false,
		// "gray");

		append("Type", type, "", false, null);
		if (name != null)
			append("Name", name, "", false, null);
		if (weight != null)
			append("Weight", weight, "", false, null);
		if (polarity != null)
			append("Polarity", polarity, "", false, null);
		if (date_start != null)
			append("Date_start", date_start, "", false, null);
		if (date_end != null)
			append("Date_end", date_end, "", false, null);
		if (valid != null)
			append("Valid", valid, "", false, null);
			...
			n.edges().forEach(e -> {
				e.attributeKeys().forEach(att -> {
					if (att.compareTo("o") == 0 || att.compareTo("s") == 0 || att.compareTo("p") == 0)
						try {
							append(att, e.getTargetNode().getAttribute("name").toString(), e.getTargetNode().getId(),
									false, "gray");
	...
	private void conflicts(Node n) throws BadLocationException, IOException {
		...
		n.edges().forEach(e -> {
			e.attributeKeys().forEach(att -> {
				if (att.compareTo("o") == 0 || att.compareTo("s") == 0 || att.compareTo("p") == 0
						|| att.compareTo("ui.class") == 0 || att.compareTo("ui.hide") == 0)
					return;
